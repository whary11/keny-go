...
func (db *database) upConnectionMysql(info *infoDatabase) (err error) {
	...
	fmt.Prinln(fmt.Sprintf("%s:%s@tcp(%s:%s)/%s", info.Read.Username, info.Read.Password, info.Read.Hostname, info.Read.Port, info.Read.Name))
	...
	fmt.Prinln(fmt.Sprintf("%s:%s@tcp(%s:%s)/%s", info.Write.Username, info.Write.Password, info.Write.Hostname, info.Write.Port, info.Write.Name))
	return
}
